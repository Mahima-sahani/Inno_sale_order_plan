...
class ReportReceive_challan(models.AbstractModel):
    _name = 'report.inno_finishing.report_print_receive_challan'
    _description = 'Will prepare the data for displaying the template.'

    @api.model
    def _get_report_values(self, docids, data=None):
        record = self.env['finishing.baazar'].sudo().browse(docids)
        rec = record.jobwork_received_ids.filtered(lambda bl: bl.state == 'verified')
        ...
        products = [{
            ...
            'penalty' : sum(
                (rec.filtered(lambda code: jobwork.id in code.product_id.ids)).mapped('penalty')),
            'cancel_pcs': '-', 'area': sum(
        ...
        data = {'company': {'company_name': record.finishing_work_id.company_id.name,
                ...
                'subcontractor': {'name': record.subcontractor_id.name, 'address': record.subcontractor_id.street,
                                  ...
                                  'invoice' : record.bill_id.name,
                                  'due_date': record.finishing_work_id.expected_date,
                                  'aadhar_no': record.subcontractor_id.vat or 'N/A', 'issue_by': self.env.user.name
                                  },
                'products': {'data': products ,'total_pcs': len(record.jobwork_received_ids.filtered(lambda st: st.state == 'verified')),
                             'penality': sum(rec.mapped('penalty')), 'total_area': sum(rec.mapped('total_area')),},
                'site': 'Main',
